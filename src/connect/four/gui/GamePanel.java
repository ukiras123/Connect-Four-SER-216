...
public class GamePanel extends javax.swing.JPanel implements ScoreChart.Listener {
	...
	@SuppressWarnings("unchecked")
        ...
        private void initComponents() {
                ...
                bgImage.setIcon(new javax.swing.ImageIcon(getClass().getResource("/board.png"))); 
	...
	void dropPiece(){
		...
		Timer timer = new Timer((8000/(destination)), new ActionListener() {
			...
			int piecesX = pieces[turnNum].getX();
			int piecesY = pieces[turnNum].getY();
			int startY = piecesY;
			...
			piecesY = piecesY+(int)Math.round((destination - startY) * progress);
			pieces[turnNum].setLocation(piecesX, piecesY);
			if(piecesY == getTargetY()){
				((Timer)(event.getSource())).stop();
				falling = false;
				System.out.println("Piece fell.");
				if(!isComputerEnabled){
	...
	public void calcWidth(int columnEntered){
		...
		if(falling == false){
			...
			pieces[turnNum].setLocation(posX,0);
			revalidate();
			repaint();
		}
	}
		...
		if(!justWon){
			if(falling == false){
				...
				if(turnNum == 42){
					...
					pnameDisplay.setText(game.getCurrentPlayer().
							getName() + "'s turn.");
